...
struct VideoEditorView: View {
    ...
    /// True when the editor was opened from the home screen to edit a saved project.
    var isEditingExistingProject: Bool = false
    ...
    /// Lightweight snapshot for change detection (order + trim points).
    ...
    private var currentClipSnapshot: [ClipSnapshot] {
    ...
    var body: some View {
        ...
        .confirmationDialog("Save Changes?", isPresented: $showExitDialog, titleVisibility: .visible) {
            ...
            Text("You have unsaved edits. Save your progress to the Home screen, or discard your changes?")
        }
                ...
                if exportSession.status == .completed {
                    ...
                    try await MainActor.run {
                        ...
                        if let existingProject = editingProject {
                            ...
                            editingProject = newProject // Link binding to new project
                        }
                        
                        do {
                            try modelContext.save()
                            editingProject = nil // Reset editing project binding only on successful save
                            isExporting = false
                            alertTitle = "Success!"
    ...
    private func saveProjectProgress() {
        ...
        Task {
            do {
                ...
                try await MainActor.run {
                    ...
                    if let existingProject = editingProject {
                        // Identify clip files that are no longer used and delete them
                        let oldClipPaths = existingProject.safeClipPaths
                        for oldPath in oldClipPaths {
                            if !finalClipPaths.contains(oldPath) {
                                let oldClipURL = documentsURL.appendingPathComponent(oldPath)
                        ...
                        // Update project details (keep existing videoPath if any)
                        existingProject.clipPaths = finalClipPaths
                        existingProject.clipTitles = finalClipTitles
                        existingProject.clipStartTimes = finalClipStartTimes
                        existingProject.clipEndTimes = finalClipEndTimes
                        ...
                        let newProject = Project(
                            name: newName,
                            videoPath: nil, // Not exported yet
                            clipPaths: finalClipPaths,
                            clipTitles: finalClipTitles,
                            clipStartTimes: finalClipStartTimes,
                            clipEndTimes: finalClipEndTimes,
                    ...
                    // Reset/exit navigation back to Home screen
                    navigationPath = NavigationPath()
                }
            } catch {
                await MainActor.run {
